...
/**
 * Configuration roles available for web/REST microservice.
 * 
 * @author Derek
 */
public enum WebRestRoles implements IConfigurationRoleProvider {

    /** Root user management role. */
    WebRest(ConfigurationRole.build(WebRestRoleKeys.WebRest, "Web/REST", false, false, false, new IRoleKey[0],
	    new IRoleKey[0], true));

    private ConfigurationRole role;

    private WebRestRoles(ConfigurationRole role) {
	this.role = role;
    }
