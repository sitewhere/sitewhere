...
public class AssetManagementModel extends ConfigurationModel {
	...
	protected void addCommonAssetModuleAttributes(ElementNode.Builder builder) {
		builder.attribute((new AttributeNode.Builder("Module id", "moduleId", AttributeType.String).description("Unique id used to reference the asset module").build()));
	}
	...
	protected ElementNode createWso2IdentityAssetModuleElement() {
		ElementNode.Builder builder =
				new ElementNode.Builder("WSO2 Identity Asset Module",
		...
		builder.attribute((new AttributeNode.Builder("SCIM users URL", "scimUsersUrl", AttributeType.String).description(
				"SCIM URL for accessing the list of users.").defaultValue(
				"https://localhost:9443/wso2/scim/Users").build()));
		builder.attribute((new AttributeNode.Builder("Username", "username", AttributeType.String).description(
				"Basic authentication username for web service calls.").defaultValue("admin").build()));
		builder.attribute((new AttributeNode.Builder("Password", "password", AttributeType.String).description(
				"Basic authentication password for web service calls.").defaultValue("admin").build()));
		builder.attribute((new AttributeNode.Builder("Ignore bad certificate", "ignoreBadCertificate",
				AttributeType.Boolean).description(
				"Indicates whether an invalid SSL certificate on the server should be ignored. "
						+ "Do not enable in production systems.").defaultValue("false").build()));
	...
	protected void addCommonFilesystemAssetModuleAttributes(ElementNode.Builder builder) {
		builder.attribute((new AttributeNode.Builder("Module name", "moduleName", AttributeType.String).description("Name shown for module in user interface.").build()));
		builder.attribute((new AttributeNode.Builder("Filename", "filename", AttributeType.String).description("Name of XML file found in the configuration assets folder.").build()));
	}
	...
	protected ElementNode createXmlDeviceAssetModuleElement() {
		ElementNode.Builder builder =
				new ElementNode.Builder("XML Device Asset Module",
	...
	protected ElementNode createXmlHardwareAssetModuleElement() {
		ElementNode.Builder builder =
				new ElementNode.Builder("XML Hardware Asset Module",
						AssetManagementParser.Elements.FilesystemHardwareAssetModule.getLocalName(),
						"laptop", ElementRole.AssetManagment_AssetModule);
	...
	protected ElementNode createXmlPersonAssetModuleElement() {
		ElementNode.Builder builder =
				new ElementNode.Builder("XML Person Asset Module",
	...
	protected ElementNode createXmlLocationAssetModuleElement() {
		ElementNode.Builder builder =
				new ElementNode.Builder("XML Location Asset Module",
						AssetManagementParser.Elements.FilesystemLocationAssetModule.getLocalName(),
						"map-pin", ElementRole.AssetManagment_AssetModule);
