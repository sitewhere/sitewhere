...
/**
 * Introspects the REST controllers to pull out documentation and generate it
 * into an HTML file.
 * 
 * @author Derek
 */
public class RestDocumentationGenerator {
	...
	System.out.println(
		"Found " + controllers.size() + " documented controllers containing " + methodCount + " methods.");
	return results;
    }
	...
	for (ParsedController controller : controllers) {
	    String globalClass = (controller.isGlobal() ? " style=\"font-style: normal;font-weight: bold;\" " : "");
	    html += "<li" + ((!setActive) ? " class=\"active\"" : "") + "><a href=\"#" + controller.getResource() + "\""
		    + globalClass + "><i class=\"icon-chevron-right\"></i> " + controller.getName() + "</a>\n";
	    html += "<ul class=\"nav\">\n";
	    for (ParsedMethod method : controller.getMethods()) {
		html += "<li><a href=\"#" + method.getName() + "\"><i class=\"icon-chevron-right\"></i> "
			+ method.getSummary() + "</a>\n";
	    }
	    html += "</ul>\n";
	    html += "</li>";
	...
	for (ParsedMethod method : controller.getMethods()) {
	    ...
	    String methodHtml = "<a id=\"" + method.getName() + "\" style=\"display:block;\">&nbsp;</a>\n"
		...
		for (ParsedExample example : method.getExamples()) {
			...
			exampleHtml += "<pre><code class='json'>" + example.getJson()
	...
	String uri = "<h3>Request URI</h3><div style=\"background-color: " + colors.getBgColor()
	...
	String table = "<h3>Path Parameters</h3><table class=\"param-table\"><thead><tr style=\"background-color: "
		+ colors.getTagColor() + "; color: #fff;\"><th>Name</th>" + "<th>Description</th></thead><tbody>";
	for (ParsedParameter param : method.getParameters()) {
	    if (param.getType() == ParameterType.Path) {
		table += "<tr style=\"background-color: " + colors.getBgColor() + "\"><td style=\"border: 1px solid "
			+ colors.getBrdColor() + ";\">" + param.getName() + "</td><td>" + param.getDescription()
			+ "</td></tr>";
		pathParamsCount++;
	    }
	}
	...
	if (breakdown.getNonConcernParameters().size() > 0) {
	    String table = "<h3>Request Parameters</h3><table class=\"param-table\"><thead><tr><th style=\"width: 25%\">Name</th>"
		    + "<th style=\"width: 50%\">Description</th><th>Required</th></thead><tbody>";

	    for (ParsedParameter param : breakdown.getNonConcernParameters()) {
		table += "<tr><td>" + param.getName() + "</td><td>" + param.getDescription() + "</td><td>"
	...
	for (ConcernType type : breakdown.getConcernParameters().keySet()) {
	    ...
	    String table = "<h3><a href=\"#" + type.getLink() + "\">" + type.getTitle()
     ...
     * @throws SiteWhereException
     */
    protected static ParsedMethod parseMethod(String baseUri, Method method, File resources) throws SiteWhereException {
