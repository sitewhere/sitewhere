...
/**
 * Specific script engine able to reload modified scripts as well as dealing
 * properly with dependent scripts.
 *
 * @author sam
 * @author Marc Palmer
...
public class SwGroovyScriptEngine implements ResourceConnector {

    private static Logger LOGGER = LoggerFactory.getLogger(SwGroovyScriptEngine.class);
	...
	protected CompilationUnit createCompilationUnit(CompilerConfiguration configuration, CodeSource source) {
		...
		public void call(final SourceUnit source, GeneratorContext context, ClassNode classNode)
		    ...
		    // GROOVY-4013: If it is an inner class, tracking its dependencies doesn't
		    // really
		    // serve any purpose and also interferes with the caching done to track
		    // dependencies
		...
		public LookupResult findClassNode(String origName, CompilationUnit compilationUnit) {
			...
			try {
			    ...
			    if (SwGroovyScriptEngine.this.isSourceNewer(entry)) {
				try {
	...
	private void updateLocalDependencyCache(GroovyCodeSource codeSource, LocalData localData) {
		...
		for (String depName : origDep) {
		    ...
		    try {
			if (origEntry == dep || SwGroovyScriptEngine.this.isSourceNewer(dep)) {
			    newDep.add(depName);
			}
		    } catch (ResourceException re) {
	...
	private String getPath(Class clazz, Map<String, String> precompiledEntries) {
		...
		if (path == null)
		    throw new GroovyBugError("Precompiled class " + name
			    + " should be available in precompiled entries map, but was not.");
		return path;
	    } else {
    ...
    /**
     * Simple testing harness for the GSE. Enter script roots as arguments and then
     * input script names to run them.
     *
     * @param urls
     *            an array of URLs
     * @throws Exception
	...
	SwGroovyScriptEngine gse = new SwGroovyScriptEngine(urls);
    ...
    /**
     * Initialize a new GroovyClassLoader with a default or constructor-supplied
     * parentClassLoader.
     *
	...
	GroovyClassLoader groovyClassLoader = (GroovyClassLoader) AccessController.doPrivileged(new PrivilegedAction() {
	    public Object run() {
     ...
     * script from the <code>ResourceConnector</code>.
     *
     * @param resourceName
	...
	for (URL root : roots) {
		...
		LOGGER.info(String.format("Loading script from root[%s] res[%s] url[%s].", root.toString(),
			resourceName, scriptURL.toString()));
	...
	LOGGER.info(String.format("Opening connection to '%s'.", scriptURL.toString()));
	URLConnection urlConnection = scriptURL.openConnection();
	LOGGER.info(String.format("Opened '%s'.", scriptURL.toString()));
    ...
    /**
     * This method closes a {@link URLConnection} by getting its {@link InputStream}
     * and calling the {@link InputStream#close()} method on it. The
     * {@link URLConnection} doesn't have a close() method and relies on garbage
     * collection to close the underlying connection to the file. Relying on garbage
     * collection could lead to the application exhausting the number of files the
     * user is allowed to have open at any one point in time and cause the
     * application to crash ({@link java.io.FileNotFoundException} (Too many open
     * files)). Hence the need for this method to explicitly close the underlying
     * connection to the file.
     *
     * @param urlConnection
     *            the {@link URLConnection} to be "closed" to close the underlying
     *            file descriptors.
     */
	...
	InputStream in = null;
	try {
	    LOGGER.info("Verifying input stream.");
    ...
    /**
     * The groovy script engine will run groovy scripts and reload them and their
     * dependencies when they are modified. This is useful for embedding groovy in
     * other containers like games and application servers.
     *
     * @param roots
     *            This an array of URLs where Groovy scripts will be stored. They
     *            should be laid out using their package structure like Java classes
     */
    private SwGroovyScriptEngine(URL[] roots, ClassLoader parent, ResourceConnector rc) {
	if (roots == null)
    ...
    public SwGroovyScriptEngine(URL[] roots) {
	this(roots, CL_STUB, null);
    }

    public SwGroovyScriptEngine(URL[] roots, ClassLoader parentClassLoader) {
	this(roots, parentClassLoader, null);
    }

    public SwGroovyScriptEngine(String[] urls) throws IOException {
    ...
    public SwGroovyScriptEngine(String[] urls, ClassLoader parentClassLoader) throws IOException {
	this(createRoots(urls), parentClassLoader, null);
    }

    public SwGroovyScriptEngine(String url) throws IOException {
	this(new String[] { url });
    }

    public SwGroovyScriptEngine(String url, ClassLoader parentClassLoader) throws IOException {
	this(new String[] { url }, parentClassLoader);
    }

    public SwGroovyScriptEngine(ResourceConnector rc) {
	this(null, CL_STUB, rc);
    }

    public SwGroovyScriptEngine(ResourceConnector rc, ClassLoader parentClassLoader) {
	this(null, parentClassLoader, rc);
    }

    /**
     * Get the <code>ClassLoader</code> that will serve as the parent ClassLoader of
     * the {@link GroovyClassLoader} in which scripts will be executed. By default,
     * this is the ClassLoader that loaded the <code>GroovyScriptEngine</code>
     * class.
     *
     * @return the parent classloader used to load scripts
     */
     ...
     * @param argument
     *            a single argument passed as a variable named <code>arg</code> in
     *            the binding
     * @return a <code>toString()</code> representation of the result of the
	...
	try {
	    ...
	    // getResourceConnection() opening the inputstream, let's ensure all streams are
	    // closed
	    forceClose(conn);
	}
	return lastMod;
