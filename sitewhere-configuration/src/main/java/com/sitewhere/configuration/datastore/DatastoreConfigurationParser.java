...
public class DatastoreConfigurationParser {
    ...
    public static DatastoreConfigurationChoice parseEventManagementDatastoreChoice(Element element,
        ...
        for (Element child : children) {
            ...
            switch (type) {
                ...
                case Warp10DBDatastore: {
                    return parseWarp10Datastore(child, context);
                }
                case Warp10DBReference: {
                    return parseWarp10Reference(child, context);
                }
     ...
     * @return
     */
    protected static DatastoreConfigurationChoice parseWarp10Datastore(Element element, ParserContext context) {
        BeanDefinitionBuilder configuration = BeanDefinitionBuilder.rootBeanDefinition(Warp10Configuration.class);
        parseWarp10Attributes(element, context, configuration);
        return new DatastoreConfigurationChoice(DatastoreConfigurationType.Warp10DB,
     ...
     * @return
     */
    protected static DatastoreConfigurationChoice parseWarp10Reference(Element element, ParserContext context) {
        Attr id = element.getAttributeNode("id");
        if (id == null) {
            throw new RuntimeException("No id specified for Cassandra configuration.");
        }
        String reference = InstanceManagementBeans.BEAN_WARP10_CONFIGURATION_BASE + id.getValue();
        return new DatastoreConfigurationChoice(DatastoreConfigurationType.Warp10DBReference, reference);
    }


    /**
     * Parse common Warp 10 configuration attributes.
     *
     * @param element
     * @param context
     */
    public static void parseWarp10Attributes(Element element, ParserContext context,
                                             BeanDefinitionBuilder configuration) {
        Attr hostname = element.getAttributeNode("hostname");
        if (hostname != null) {
            configuration.addPropertyValue("hostname", hostname.getValue());
        ...
        Attr tokenSecret = element.getAttributeNode("tokenSecret");
        if (tokenSecret != null) {
            configuration.addPropertyValue("tokenSecret", tokenSecret.getValue());
        }
