...
public class ClientTracingInterceptor implements ClientInterceptor {
    ...
    private ClientTracingInterceptor(Tracer tracer, OperationNameConstructor operationNameConstructor,
	    boolean streaming, boolean verbose, Set<ClientRequestAttribute> tracedAttributes,
	 ...
	 *            to name all spans created by this intercepter
	...
	/**
	 * @param tracedAttributes
	 *            to set as tags on client spans created by this intercepter
	 ...
	 *            that provides a method of getting the active span before
	 *            the client call
	 * @return this Builder configured to start client span as children of
	 *         the span returned by activeSpanSource.getActiveSpan()
	 */
	...
	public ClientTracingInterceptor build() {
	    return new ClientTracingInterceptor(this.tracer, this.operationNameConstructor, this.streaming,
		    this.verbose, this.tracedAttributes, this.activeSpanSource);
	}
    }

    public enum ClientRequestAttribute {
	METHOD_TYPE, METHOD_NAME, DEADLINE, COMPRESSOR, AUTHORITY, ALL_CALL_OPTIONS, HEADERS
    }
}