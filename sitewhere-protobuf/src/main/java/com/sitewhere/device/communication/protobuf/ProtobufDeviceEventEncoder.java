...
public class ProtobufDeviceEventEncoder implements IDeviceEventEncoder<byte[]> {
		...
		try {
			...
			if (measurements.getMetadata() != null) {
				...
				for (String key : metaKeys) {
					mb.addMetadata(
							Model.Metadata.newBuilder().setName(key).setValue(
									measurements.getMetadata().get(key))).build();
				}
			}
	...
	/**
	 * Encode a {@link IDecodedDeviceRequest} containing an alert in a protobuf message.
	 * 
	 * @param request
	 * @return
	 * @throws SiteWhereException
	...
	protected byte[] encodeDeviceAlert(IDecodedDeviceRequest<IDeviceAlertCreateRequest> event)
		...
		try {
			...
			if (alert.getMetadata() != null) {
				...
				for (String key : metaKeys) {
					mb.addMetadata(
							Model.Metadata.newBuilder().setName(key).setValue(alert.getMetadata().get(key))).build();
				}
			}
	...
	/**
	 * Encode a {@link IDecodedDeviceRequest} containing a location in a protobuf message.
	 * 
	 * @param request
	 * @return
	 * @throws SiteWhereException
	...
	protected byte[] encodeDeviceLocation(IDecodedDeviceRequest<IDeviceLocationCreateRequest> event)
		...
		try {
			...
			if (location.getMetadata() != null) {
				...
				for (String key : metaKeys) {
					mb.addMetadata(
							Model.Metadata.newBuilder().setName(key).setValue(location.getMetadata().get(key))).build();
				}
			}
