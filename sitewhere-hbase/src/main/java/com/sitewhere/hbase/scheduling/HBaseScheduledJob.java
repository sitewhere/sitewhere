...
public class HBaseScheduledJob {
	...
	public static ScheduledJob createScheduledJob(IHBaseContext context, IScheduledJobCreateRequest request)
		...
		return HBaseUtils.createOrUpdate(context, context.getPayloadMarshaler(),
				ISiteWhereHBase.SCHEDULES_TABLE_NAME, job, uuid, KEY_BUILDER, qualifiers);
	}
	...
	public static IScheduledJob updateScheduledJob(IHBaseContext context, String token,
		...
		SiteWherePersistence.scheduledJobUpdateLogic(updated, request);
		return HBaseUtils.put(context, context.getPayloadMarshaler(), ISiteWhereHBase.SCHEDULES_TABLE_NAME,
	 ...
	 * @throws SiteWhereException
	 */
	public static ScheduledJob getScheduledJobByToken(IHBaseContext context, String token)
			throws SiteWhereException {
		if (KEY_BUILDER.getMap(context).getValue(token) == null) {
			return null;
		}
		return HBaseUtils.get(context, ISiteWhereHBase.SCHEDULES_TABLE_NAME, token, KEY_BUILDER,
				ScheduledJob.class);
	}
	 ...
	 * @throws SiteWhereException
	 */
	public static SearchResults<IScheduledJob> listScheduledJobs(IHBaseContext context,
		...
		return HBaseUtils.getFilteredList(context, ISiteWhereHBase.SCHEDULES_TABLE_NAME, KEY_BUILDER,
				includeDeleted, IScheduledJob.class, ScheduledJob.class, filter, criteria, comparator);
	}
	...
	public static IScheduledJob deleteScheduledJob(IHBaseContext context, String token, boolean force)
		...
		return HBaseUtils.delete(context, context.getPayloadMarshaler(),
				ISiteWhereHBase.SCHEDULES_TABLE_NAME, token, force, KEY_BUILDER, ScheduledJob.class);
	}

	/**
	 * Get a {@link ScheduledJob} by token or throw an exception if token is not valid.
	 * 
	 * @param context
	 * @param token
	 * @return
	 * @throws SiteWhereException
	 */
	public static ScheduledJob assertScheduledJob(IHBaseContext context, String token)
			throws SiteWhereException {
